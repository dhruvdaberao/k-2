...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'insert') {
      const { error } = await supabaseAdmin.from('hero_slides').insert(payload);
      ...
      await Promise.all(payload.map((u: any) => 
        supabaseAdmin.from('hero_slides').update({ position: u.position }).eq('id', u.id)
      ));
    } else {
    ...
    console.error('[Admin Catalog API] POST Carousel Error:', err);
...
export async function DELETE(request: Request) {
  try {
    ...
    console.error('[Admin Catalog API] DELETE Carousel Error:', err);
    return NextResponse.json({ success: false, error: 'Internal Server Error' }, { status: 500 });
  }
}
